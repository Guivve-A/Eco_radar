...
class AudioAnalysisWindow(QMainWindow):
    ...
    def _build_step_zone(self) -> QWidget:
        ...
        self.use_zone_check = QCheckBox("Usar zona para filtrar especies")
        self.use_zone_check.toggled.connect(self._validate_zone_inputs)
        ...
        grid.addWidget(self.use_zone_check, 0, 0, 1, 4)
        grid.addWidget(QLabel("Pais"), 1, 0)
        grid.addWidget(self.country_edit, 1, 1)
        grid.addWidget(QLabel("Provincia"), 1, 2)
        grid.addWidget(self.province_edit, 1, 3)
        grid.addWidget(QLabel("Canton/Ciudad"), 2, 0)
        grid.addWidget(self.canton_edit, 2, 1)
        grid.addWidget(QLabel("Latitud"), 2, 2)
        grid.addWidget(self.lat_spin, 2, 3)
        grid.addWidget(QLabel("Longitud"), 3, 0)
        grid.addWidget(self.lon_spin, 3, 1)
        grid.addWidget(QLabel("Radio"), 3, 2)
        grid.addWidget(self.radius_spin, 3, 3)
        grid.addWidget(self.zone_validation_label, 4, 0, 1, 4)
    ...
    def _configure_tab_order(self) -> None:
        ...
        QWidget.setTabOrder(self.output_path_edit, self.use_zone_check)
        QWidget.setTabOrder(self.use_zone_check, self.country_edit)
        ...
        QWidget.setTabOrder(self.threads_spin, self.run_btn)
    ...
    def _validate_zone_inputs(self) -> bool:
        if not self.use_zone_check.isChecked():
            self.zone_validation_label.setObjectName("HintLabel")
            self.zone_validation_label.setText("Filtro de zona desactivado.")
    ...
    def _refresh_profile_library(self) -> None:
        ...
        for idx, profile in enumerate(profiles):
            ...
            self.profile_table.setItem(idx, 2, QTableWidgetItem(str(profile.embedding_dim)))
    ...
    def _add_reference_species(self) -> None:
        ...
        try:
            ...
            profile, chunks = self.audio_analyzer.add_reference_profile(name, file_path)
    ...
    def _command_args(self) -> list[str]:
        ...
        if self.use_zone_check.isChecked():
            args.extend(
                [
                    "--lat",
        ...
        if self.export_options.include_zone_metadata and "csv" in self.export_options.formats and self.use_zone_check.isChecked():
    ...
    def _start_analysis(self) -> None:
        ...
        if not self.profile_manager.list_profiles():
            ...
            QMessageBox.warning(
                self,
                "Perfiles requeridos",
                "Debes agregar al menos una especie en 'Biblioteca de Especies' antes de analizar.",
            )
            return
        ...
        zone_meta = None
        if self.export_options.include_zone_metadata and self.use_zone_check.isChecked():
            zone_meta = {
    ...
    def _populate_detections_table(self, rows: list[dict]) -> None:
        ...
        for idx, row in enumerate(rows):
            ...
            self.detections_table.setItem(idx, 0, QTableWidgetItem(f"{row['start']:.2f}"))
            self.detections_table.setItem(idx, 1, QTableWidgetItem(f"{row['end']:.2f}"))
            self.detections_table.setItem(idx, 2, QTableWidgetItem(row["common"]))
            self.detections_table.setItem(idx, 3, QTableWidgetItem(row["scientific"]))
            self.detections_table.setItem(idx, 4, QTableWidgetItem(f"{row['confidence']:.3f}"))
            self.detections_table.setItem(idx, 5, QTableWidgetItem(row["file"]))
    ...
    def _load_settings(self) -> None:
        ...
        self.use_zone_check.setChecked(self.settings.value("use_zone", False, type=bool))
    ...
    def closeEvent(self, event) -> None:
        ...
        self.settings.setValue("use_zone", self.use_zone_check.isChecked())
